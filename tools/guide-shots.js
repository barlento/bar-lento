...
async function signAll(p){ for(let i=0;i<6;i++){ await p.waitForFunction(()=>document.getElementById("rulesOverlay").classList.contains("show"),null,{timeout:10000}); const title=await p.textContent("#rulesTitle"); if(/Training/.test(title)){ await p.click("#rulesLater"); return; } await p.evaluate(()=>{const x=document.getElementById("rulesBody"); x.scrollTop=x.scrollHeight;}); await p.waitForTimeout(350); if(await p.isVisible("#rulesEmailField") && !(await p.inputValue("#rulesEmail"))) await p.fill("#rulesEmail","joe@example.com"); await p.check("#rulesCheck"); await p.click("#rulesAgree"); await p.waitForFunction((t)=>document.getElementById("rulesTitle").textContent!==t||!document.getElementById("rulesOverlay").classList.contains("show"),title,{timeout:10000}); } }
(async()=>{
  ...
  await p.click('.who-name[data-name="Joe"]'); await p.waitForFunction(()=>/PIN/.test(document.getElementById("pinTitle").textContent)); await p.waitForTimeout(400);
  ...
  await p.waitForSelector("#rulesOverlay.show"); await p.waitForTimeout(500);
  await p.screenshot({path:OUT+"/03-rules.png"});
  await p.evaluate(()=>{const x=document.getElementById("rulesBody"); x.scrollTop=x.scrollHeight;}); await p.waitForTimeout(400); await p.fill("#rulesEmail","joe@example.com"); await p.check("#rulesCheck"); await p.click("#rulesAgree");
  await p.waitForFunction(()=>/Your documents/.test(document.getElementById("rulesTitle").textContent)); await p.waitForTimeout(400);
  await p.evaluate(()=>{const x=document.getElementById("rulesBody"); x.scrollTop=x.scrollHeight;}); await p.waitForTimeout(500);
  ...
  await q.goto(B+"/?v=g2"); await q.waitForSelector("#whoOverlay.show"); await q.click('.who-name[data-name="Pietro"]'); await q.waitForFunction(()=>/PIN/.test(document.getElementById("pinTitle").textContent));
