...
(async()=>{
  ...
  const ui=await p.evaluate(()=>({admin:document.body.classList.contains("admin"), chef:document.body.classList.contains("chef"), mode:document.querySelector("#adminBar [data-t=adminMode]").textContent, names:[...new Set([...document.querySelectorAll(".shift .who")].map(e=>e.firstChild.textContent.trim()))], history:getComputedStyle(document.getElementById("historyBtn")).display, del:getComputedStyle(document.getElementById("deleteWeekBtn")).display, tools:getComputedStyle(document.getElementById("weekTools")).display, daybtn:getComputedStyle(document.querySelector(".daybtn")).display, strip:document.getElementById("meStrip").className}));
  ...
  if(!ui.admin||!ui.chef||!/Chef mode/.test(ui.mode)||ui.names.some(n=>!inK(n))||ui.history!=="none"||ui.tools==="none"||ui.del!=="none"||ui.daybtn!=="none"||!/show/.test(ui.strip)) errors.push("chef mode after PIN wrong (New week allowed, Delete week not): "+JSON.stringify(ui));
  ...
  // the chef may create a new week that holds kitchen shifts only (plans the kitchen before Marta plans the floor)
  { const nw=(()=>{const d=new Date(wk+"T12:00:00Z"); d.setUTCDate(d.getUTCDate()+7); return d.toISOString().slice(0,10);})();
    const dn=JSON.parse(JSON.stringify(r.j.data)); dn.weeks[nw]={notes:{},mon:[{id:"chef-nw",name:"Catherine",start:"10:00",end:"14:00",station:"K"}],tue:[],wed:[],thu:[],fri:[],sat:[],sun:[]};
    const rn=await j("/api/data",{method:"POST",headers:C,body:JSON.stringify({version:r.j.version,data:dn})}); console.log("6b chef creates next week (kitchen only):",rn.s); if(rn.s!==200) errors.push("chef could not create a kitchen-only week: "+JSON.stringify(rn.j));
    const dz=JSON.parse(JSON.stringify(rn.j.data)); dz.weeks[nw].tue.push({id:"chef-nw2",name:F,start:"10:00",end:"14:00",station:"S1"}); dz.weeks[nw].notes.wed={status:"closed",text:""};
    const rz=await j("/api/data",{method:"POST",headers:C,body:JSON.stringify({version:rn.j.version,data:dz})}); console.log("6c chef adds a floor shift + closes a day in it:",rz.s); if(rz.s!==403) errors.push("chef could change the floor/day settings of the new week");
    r=rn; }
  ...
  const e1=await fetch(B+"/api/export?person="+encodeURIComponent(F),{headers:C}); const e2=await fetch(B+"/api/export?person=Catherine",{headers:C}); const e3=await fetch(B+"/api/export",{headers:C}); console.log("12 export floor:",e1.status,"kitchen:",e2.status,"archive:",e3.status,"(all 403: reports are the owner's)"); if(e1.status!==403||e2.status!==403||e3.status!==403) errors.push("export scope wrong");
  ...
  console.log("22 View as Mariia (chef):",va,vw); if(!va||!vw.bar||vw.names.some(n=>!inK(n))||vw.addshift==="none") errors.push("view-as wrong (the chef's tools must show): "+JSON.stringify(vw));
