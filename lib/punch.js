...
async function appClockNames(data) {
  ...
  if (toast.enabled()) {
    try { const emps = await toast.employees(true); const map = toast.autoMap(data.staff, data.toastMap, emps); const byGuid = Object.fromEntries(emps.map((e) => [e.guid, e])); (data.staff || []).forEach((n) => { if (!map[n]) set.add(n); else if (byGuid[map[n]] && byGuid[map[n]].salaried) set.add(n); }); } catch (e) {}
  }
  return set;
}
...
module.exports = { isAppClock, appClockNames, archivedFor, all, openEntry, punch, between, weekEntries, month, minutes, archive };
