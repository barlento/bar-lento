...
function view(e, now) {
  ...
  return Object.assign({}, e, { breakMin: Math.round(breakMinutes(e, now)), breakStart: ob && !e.out ? ob.start : undefined, breaks: (e.breaks || []).map((b) => ({ start: b.start, end: b.end || e.out || null, auto: b.auto })) });
}
...
function minutes(entries, now) {
  const t = now || Date.now();
  return entries.reduce((s, e) => s + Math.max(0, (Date.parse(e.out || new Date(t).toISOString()) - Date.parse(e.in)) / 60000 - (typeof e.breakMin === "number" ? e.breakMin : breakMinutes(e, t))), 0);
}
