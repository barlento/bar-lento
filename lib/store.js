...
async function appendPending(changes) {
  if (!hasStorage() || !changes.length) return;
  await redis("RPUSH", PENDING_KEY, ...changes.map(String));
  await redis("LTRIM", PENDING_KEY, -300, -1);
  await redis("SET", PENDING_SINCE_KEY, new Date().toISOString(), "NX");
}
async function getPending() {
  if (!hasStorage()) return { changes: [], since: null };
  const [changes, since] = await Promise.all([redis("LRANGE", PENDING_KEY, 0, -1), redis("GET", PENDING_SINCE_KEY)]);
  return { changes: changes || [], since: since || null };
}
async function clearPending() {
  if (!hasStorage()) return;
  await redis("DEL", PENDING_KEY, PENDING_SINCE_KEY);
}
