...
function normalizeShift(s, idx) {
  ...
  if (s.src === "toast" || s.src === "app") out.src = s.src; // a real clock-in (Toast POS or the app clock) written into a past week, not a shift planned by the manager
...
function normalizeData(raw) {
  const data = { staff: [], birthdays: {}, toastMap: {}, toastIgnore: [], appClock: [], dept: {}, deptManual: [], weeks: {} };
  if (Array.isArray(raw.appClock)) data.appClock = raw.appClock.map((n) => String(n || "").trim().slice(0, 60)).filter(Boolean).slice(0, 80);
  if (raw.dept && typeof raw.dept === "object") Object.keys(raw.dept).forEach((n) => { const v = raw.dept[n]; if (v === "kitchen" || v === "floor" || v === "management" || v === "owner" || v === "chef") data.dept[String(n).trim().slice(0, 60)] = v; }); // floor, kitchen or management; unset = floor
  data.deptManual = Array.isArray(raw.deptManual) ? raw.deptManual.map((n) => String(n || "").trim().slice(0, 60)).filter(Boolean).slice(0, 80) : []; // names whose department the manager chose by hand (Toast never overrides them)
  if (Array.isArray(raw.toastIgnore)) data.toastIgnore = raw.toastIgnore.map((v) => String(v || "").trim()).filter((v) => /^[A-Za-z0-9-]{8,64}$/.test(v)).slice(0, 200);
  if (raw.toastMap && typeof raw.toastMap === "object") {
    Object.keys(raw.toastMap).slice(0, 200).forEach((name) => {
      const v = String(raw.toastMap[name] || "").trim();
      if (/^[A-Za-z0-9-]{8,64}$/.test(v)) data.toastMap[String(name).trim().slice(0, 60)] = v;
    });
  }
...
// ---- pending notification (changes accumulate, one push goes out) ----
const PENDING_KEY = "barlento:pending";
const PENDING_SINCE_KEY = "barlento:pending_since";
const PENDING_LAST_KEY = "barlento:pending_last";
// items: {kind:"newweek", week} | {kind:"update", text, names[]} — one JSON string each
async function appendPending(items) {
  if (!hasStorage() || !items.length) return;
  await redis("RPUSH", PENDING_KEY, ...items.map((x) => JSON.stringify(x)));
  await redis("LTRIM", PENDING_KEY, -300, -1);
  const now = new Date().toISOString();
  await redis("SET", PENDING_SINCE_KEY, now, "NX");
  await redis("SET", PENDING_LAST_KEY, now); // the manager is still editing: delivery waits for a quiet spell
}
async function getPending() {
  if (!hasStorage()) return { changes: [], since: null, last: null };
  const [raw, since, last] = await Promise.all([redis("LRANGE", PENDING_KEY, 0, -1), redis("GET", PENDING_SINCE_KEY), redis("GET", PENDING_LAST_KEY)]);
  const changes = (raw || []).map((x) => { try { const o = JSON.parse(x); return o && o.kind ? o : null; } catch (e) { return /^NEWWEEK:/.test(x) ? { kind: "newweek", week: x.slice(8) } : null; } }).filter(Boolean);
  return { changes, since: since || null, last: last || since || null };
}
async function clearPending() {
  if (!hasStorage()) return;
  await redis("DEL", PENDING_KEY, PENDING_SINCE_KEY, PENDING_LAST_KEY);
}

// ---- site-wide announcement (e.g. "new week posted"), shown once per device ----
const ANNOUNCE_KEY = "barlento:announce";
async function setAnnouncement(a) { if (hasStorage()) await redis("SET", ANNOUNCE_KEY, JSON.stringify(a)); }
