...
function normalizeData(raw) {
  const data = { staff: [], birthdays: {}, toastMap: {}, toastIgnore: [], appClock: [], dept: {}, weeks: {} };
  if (Array.isArray(raw.appClock)) data.appClock = raw.appClock.map((n) => String(n || "").trim().slice(0, 60)).filter(Boolean).slice(0, 80);
  if (raw.dept && typeof raw.dept === "object") Object.keys(raw.dept).forEach((n) => { const v = raw.dept[n]; if (v === "kitchen" || v === "floor") data.dept[String(n).trim().slice(0, 60)] = v; }); // floor or kitchen; unset = floor
