...
async function employees(includeArchived, fresh) {
  ...
  const all = await cached("employees_all", 600, async () => {
    ...
    return list.map((e) => ({ guid: e.guid, name: fullName(e), first: e.firstName || "", last: e.lastName || "", email: String(e.email || "").trim().toLowerCase(), created: e.createdDate || null, archived: e.deleted === true || Boolean(e.deletedDate) }));
  });
...
module.exports = { enabled, employees, timeEntries, dayStatus, dayBounds, restaurant, autoMap, firstMatches, levenshtein, weekEntriesFor, shiftDate, HOST };
