...
function deptOf(jobTitles) { const list = (jobTitles || []).filter(Boolean); if (!list.length) return null; if (list.some((t) => /\b(owner|titolare|propriet)/i.test(t))) return "owner"; if (list.some((t) => MGMT_RE.test(t))) return "management"; if (list.some((t) => /\b(executive|head)\s+chef\b|\bchef\s+de\s+cuisine\b|\bcapo\s*cuoco\b/i.test(t))) return "chef"; return list.some((t) => KITCHEN_RE.test(t)) ? "kitchen" : "floor"; }
...
// Breaks taken on the Toast terminal (Toast "Breaks" feature, owner 2026-09-21: show them like the app-clock ones).
// Toast: breaks:[{guid, breakType, paid, inDate, outDate, missed}]. Unpaid minutes are subtracted from the hours (breakMin);
// breakStart = a break still running (paid or not) so the app can say "on break since …".
function breakView(t) {
  const list = Array.isArray(t.breaks) ? t.breaks.filter((b) => b && b.inDate && !b.missed) : [];
  if (!list.length) return {};
  let unpaid = 0, open = null;
  list.forEach((b) => { const endISO = b.outDate || t.outDate; if (!endISO) { open = open && open < b.inDate ? open : b.inDate; return; } if (!b.paid) unpaid += Math.max(0, (Date.parse(endISO) - Date.parse(b.inDate)) / 60000); });
  return { breakMin: Math.round(unpaid), breakStart: open || undefined, breaks: list.map((b) => ({ start: b.inDate, end: b.outDate || null, paid: !!b.paid })) };
}
function normEntry(t) { return Object.assign({ guid: t.guid, employeeGuid: t.employeeReference && t.employeeReference.guid, jobGuid: t.jobReference && t.jobReference.guid, in: t.inDate || null, out: t.outDate || null }, breakView(t)); }

// Time entries between two ISO instants: [{guid, employeeGuid, in, out, jobGuid, breakMin?, breakStart?}]
async function timeEntries(startISO, endISO, ttl) {
  ...
  return cached(key, ttl || 30, async () => {
    ...
    return list.filter((t) => !t.deleted).map(normEntry);
  });
}
...
async function dayStatus(dateISO, nameToGuid) {
  ...
  entries.forEach((t) => {
    ...
    (byName[name] = byName[name] || []).push({ in: t.in, out: t.out, breakMin: t.breakMin, breakStart: t.breakStart });
  });
...
async function weekEntriesFor(guid, weekISO) {
  ...
  return entries
    ...
    .map((t) => { const date = shiftDate(t.in); const i = days.indexOf(date); return i < 0 ? null : { day: DAYS[i], date, in: t.in, out: t.out, breakMin: t.breakMin, breakStart: t.breakStart }; })
    .filter(Boolean)
...
module.exports = { breakView, normEntry, enabled, employees, jobs, deptOf, timeEntries, timeEntriesBusiness, timeEntriesDay, timeEntriesSpan, dayStatus, dayBounds, restaurant, autoMap, firstMatches, levenshtein, weekEntriesFor, shiftDate, HOST };
