...
async function employees(includeArchived) {
  const all = await cached("employees_all", 600, async () => {
    ...
    return list.map((e) => ({ guid: e.guid, name: fullName(e), first: e.firstName || "", last: e.lastName || "", email: String(e.email || "").trim().toLowerCase(), archived: e.deleted === true || Boolean(e.deletedDate) }));
  });
...
module.exports = { enabled, employees, timeEntries, dayStatus, dayBounds, restaurant, autoMap, levenshtein, weekEntriesFor, HOST };
