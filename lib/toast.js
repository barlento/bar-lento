...
async function employees(includeArchived, fresh) {
  ...
  const all = await cached("employees_all", 600, async () => {
    ...
    return list.map((e) => ({ guid: e.guid, name: fullName(e), first: e.firstName || "", last: e.lastName || "", email: String(e.email || "").trim().toLowerCase(), created: e.createdDate || null, archived: isArchived(e) }));
  });
...
module.exports = { enabled, employees, timeEntries, timeEntriesBusiness, timeEntriesDay, timeEntriesSpan, dayStatus, dayBounds, restaurant, autoMap, firstMatches, levenshtein, weekEntriesFor, shiftDate, HOST };
