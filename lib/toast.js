...
async function jobs() {
  return cached("jobs", 3600, async () => {
    ...
    const out = {}; (list || []).forEach((j) => { if (j && j.guid && j.title) out[j.guid] = String(j.title).trim(); }); return out;
  });
}
...
function deptOf(jobTitles) { const list = (jobTitles || []).filter(Boolean); if (!list.length) return null; if (list.some((t) => MGMT_RE.test(t))) return "management"; return list.some((t) => KITCHEN_RE.test(t)) ? "kitchen" : "floor"; }
...
async function employees(includeArchived, fresh) {
  ...
  const all = await cached("employees_all", 600, async () => {
    ...
    return list.map((e) => ({ guid: e.guid, name: fullName(e), first: e.firstName || "", last: e.lastName || "", email: String(e.email || "").trim().toLowerCase(), created: e.createdDate || null, archived: isArchived(e), jobs: (e.jobReferences || []).map((j) => titles[j && j.guid]).filter(Boolean) }));
  });
