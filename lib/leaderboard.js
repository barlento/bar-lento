...
function scoreRows(names, entries, guidToName, sched, nowMs) {
  ...
  entries.forEach((t) => {
    ...
    if (t.out) r.msClosed += Math.max(0, Date.parse(t.out) - inMs - (t.breakMin || 0) * 60000); else { if (!r.openSince || inMs < Date.parse(r.openSince)) r.openSince = t.in; r.msClosed -= (t.breakMin || 0) * 60000 + (t.breakStart ? Math.max(0, nowMs - Date.parse(t.breakStart)) : 0); } // unpaid breaks never count as hours
  ...
  return names.map((n) => {
    ...
    const hours = Math.max(0, r.msClosed + liveMs) / 3600000;
