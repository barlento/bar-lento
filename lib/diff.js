...
// notable   = only what matters to an employee, as {text, names}: shifts added/removed/time or name changed (the person),
//             day status changed (the people scheduled that day). Each person is notified only about their own items.
...
function diffData(oldD, newD) {
  ...
  [...weeks].sort().forEach((wk) => {
    ...
    if (!nw) {
      changes.push(`Week of ${wk}: deleted`);
      DAY_KEYS.forEach((d) => (ow[d] || []).forEach((s) => changes.push(`${dateOf(wk, d)}: removed ${shiftLabel(s)}`)));
      notable.push({ text: `Week of ${weekRangeLabel(wk)} was removed`, names: [...new Set(DAY_KEYS.flatMap((d) => (ow[d] || []).map((s) => s.name)))] });
      return;
    }
    DAY_KEYS.forEach((d) => {
      ...
      newList.forEach((s) => {
        ...
        if (!prev) { const m = `${when}: added ${shiftLabel(s)}`; changes.push(m); if (!newWeeks.includes(wk)) notable.push({ text: m, names: [s.name] }); return; }
        ...
        if (material || prev.station !== s.station) {
          ...
          changes.push(m);
          if (material) { resetKeys.push(`${wk}:${d}:${s.id}`); notable.push({ text: m, names: [...new Set([prev.name, s.name])] }); }
        }
      });
      oldList.forEach((s) => { if (!newById.has(s.id)) { const m = `${when}: removed ${shiftLabel(s)}`; changes.push(m); notable.push({ text: m, names: [s.name] }); } });
      ...
      if (os !== ns || hoursOf(on) !== hoursOf(nn)) { const m = `${when}: day set to ${STATUS_LABEL[ns] || ns || "Open"}${hoursOf(nn) ? ` (${hoursOf(nn)})` : ""}${nt ? ` — ${nt}` : ""}`; changes.push(m); if (!newWeeks.includes(wk)) notable.push({ text: m, names: [...new Set(oldList.concat(newList).map((s) => s.name))] }); } // a day change concerns the people scheduled that day
