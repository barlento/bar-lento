...
// `forOwner` = the copy that goes to the owner (RULES.copyTo): same signed text, but addressed to the owner as proof
// ("Prova acknowledged…") instead of "here is your copy".
function rulesCopy(ack, forOwner) {
  const R = RULES;
  const signer = ack.fullName || ack.name;
  const lead = forOwner
    ? `<b>${esc(signer)}</b>${ack.fullName && ack.fullName !== ack.name ? ` (schedule name: ${esc(ack.name)})` : ""} read and acknowledged the Bar Lento House Rules, version ${esc(ack.version)}, on ${esc(nyDate(ack.at))}. Their signed copy was sent to <a href="mailto:${esc(ack.email)}">${esc(ack.email)}</a>. Below is the exact text they acknowledged, with the electronic signature block. Keep this email: it is Bar Lento's record.`
    : `Hi ${esc(signer.split(/\s+/)[0])}, here is your copy of the Bar Lento House Rules you acknowledged. Keep this email: it is your signed copy.`;
  const sig = [
...
<p style="margin:16px 0 0;font-size:12px;color:#5A4A39">You can read or print the rules at any time: <a href="https://bar-lento.vercel.app/rules.html">bar-lento.vercel.app/rules.html</a></p>
  ...
  const text = [forOwner ? `${signer} acknowledged the Bar Lento House Rules (version ${ack.version}) on ${nyDate(ack.at)}. Signed copy sent to ${ack.email}.` : `Your signed copy of the Bar Lento House Rules.`, "", `${R.title} — version ${ack.version}`, R.subtitle, "", R.intro, ""]
  ...
  const subject = forOwner
    ? `✓ ${signer} acknowledged the House Rules (version ${ack.version})`
    : `Bar Lento — House Rules (version ${ack.version}) — your signed copy`;
