...
function packetCopy(docs, ack, forOwner) {
  ...
  const body = docs.map((R) => `<div style="margin-top:28px;padding-top:18px;border-top:2px solid #D6C6A6">
<h1 style="font-size:20px;margin:0 0 4px">${esc(R.icon || "")} ${esc(R.title)}</h1>
<p style="margin:0 0 12px;color:#5A4A39;font-size:14px">${esc(R.subtitle || "")} · Version ${esc(R.version)}</p>
<p style="font-size:14px">${esc(R.intro || "")}</p>
${renderSectionsHtml(R.sections || [])}
...
<p style="margin:12px 0 0;font-size:13px;font-style:italic;color:#5A4A39">${esc(R.ack)}</p></div>`).join("");
  const html = `<!doctype html><html><body style="margin:0;background:#F3EDE1;font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;color:#1F150C;line-height:1.5">
<div style="max-width:680px;margin:0 auto;padding:24px 16px">
<div style="background:#FFFCF6;border:1px solid #D6C6A6;border-radius:14px;padding:24px">
<p style="margin:0 0 14px;font-size:15px">${lead}</p>
...
${body}
${contacts ? `<div style="margin-top:18px;padding:12px 14px;border-radius:10px;background:#ECE3D2;font-size:14px"><b>Contacts</b><br>${contacts}</div>` : ""}
<div style="margin-top:18px;padding:14px 16px;border-left:4px solid #C9701E;background:#FFF7EA;font-size:14px"><p style="margin:0 0 10px;font-style:italic">I confirm that I have received and read all the documents above.</p>
<table style="font-size:13px;border-collapse:collapse">${sig.map(([k, v]) => `<tr><td style="padding:2px 12px 2px 0;color:#5A4A39;white-space:nowrap;vertical-align:top">${esc(k)}</td><td style="padding:2px 0"><b>${esc(v)}</b></td></tr>`).join("")}</table>
...
<p style="margin:16px 0 0;font-size:12px;color:#5A4A39">Printable versions: <a href="https://bar-lento.vercel.app/docs.html">bar-lento.vercel.app/docs.html</a></p>
</div></div></body></html>`;
