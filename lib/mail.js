...
function renderSectionsHtml(sections) {
  return sections.map((s) => {
    ...
    return `<h3 style="font-size:15px;margin:18px 0 4px;color:#8A3F25">${esc(s.h)}</h3>${body}`;
  }).join("");
}
...
function docCopy(doc, ack, forOwner) {
  ...
  const html = `<!doctype html><html><body style="margin:0;background:#F5F4EF;font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;color:#1F1E1B;line-height:1.5">
<div style="max-width:680px;margin:0 auto;padding:24px 16px">
<div style="background:#FFFFFF;border:1px solid #E6E3DA;border-radius:14px;padding:24px">
<p style="margin:0 0 14px;font-size:15px">${lead}</p>
<h1 style="font-size:22px;margin:0 0 4px">${esc(R.title)}</h1>
<p style="margin:0 0 12px;color:#6B6861;font-size:14px">${esc(R.subtitle || "")} · Version ${esc(ack.version)}</p>
<p style="font-size:14px">${esc(R.intro || "")}</p>
${renderSectionsHtml(R.sections || [])}
${links}
${contacts ? `<div style="margin-top:18px;padding:12px 14px;border-radius:10px;background:#F1EFE8;font-size:14px"><b>Contacts</b><br>${contacts}</div>` : ""}
<div style="margin-top:18px;padding:14px 16px;border-left:4px solid #D97757;background:#FBF3EF;font-size:14px"><p style="margin:0 0 10px;font-style:italic">${esc(R.ack)}</p>
<table style="font-size:13px;border-collapse:collapse">${sig.map(([k, v]) => `<tr><td style="padding:2px 12px 2px 0;color:#6B6861;white-space:nowrap">${esc(k)}</td><td style="padding:2px 0"><b>${esc(v)}</b></td></tr>`).join("")}</table>
<p style="margin:10px 0 0;font-size:12px;color:#6B6861">This ${training ? "confirmation" : "acknowledgment"} was given electronically in the Bar Lento schedule app after signing in with a personal PIN, and is recorded by Bar Lento with the date, time and device shown above.</p></div>
<p style="margin:16px 0 0;font-size:12px;color:#6B6861">You can read or print this document at any time: <a href="${esc(printUrl)}">${esc(printUrl.replace(/^https?:\/\//, ""))}</a></p>
</div></div></body></html>`;
...
function packetCopy(docs, ack, forOwner) {
  ...
  const body = docs.map((R) => `<div style="margin-top:28px;padding-top:18px;border-top:2px solid #E6E3DA">
<h1 style="font-size:20px;margin:0 0 4px">${esc(R.icon || "")} ${esc(R.title)}</h1>
<p style="margin:0 0 12px;color:#6B6861;font-size:14px">${esc(R.subtitle || "")} · Version ${esc(R.version)}</p>
<p style="font-size:14px">${esc(R.intro || "")}</p>
${renderSectionsHtml(R.sections || [])}
...
<p style="margin:12px 0 0;font-size:13px;font-style:italic;color:#6B6861">${esc(R.ack)}</p></div>`).join("");
  const html = `<!doctype html><html><body style="margin:0;background:#F5F4EF;font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;color:#1F1E1B;line-height:1.5">
<div style="max-width:680px;margin:0 auto;padding:24px 16px">
<div style="background:#FFFFFF;border:1px solid #E6E3DA;border-radius:14px;padding:24px">
<p style="margin:0 0 14px;font-size:15px">${lead}</p>
...
${body}
${contacts ? `<div style="margin-top:18px;padding:12px 14px;border-radius:10px;background:#F1EFE8;font-size:14px"><b>Contacts</b><br>${contacts}</div>` : ""}
<div style="margin-top:18px;padding:14px 16px;border-left:4px solid #D97757;background:#FBF3EF;font-size:14px"><p style="margin:0 0 10px;font-style:italic">I confirm that I have received and read all the documents above.</p>
<table style="font-size:13px;border-collapse:collapse">${sig.map(([k, v]) => `<tr><td style="padding:2px 12px 2px 0;color:#6B6861;white-space:nowrap;vertical-align:top">${esc(k)}</td><td style="padding:2px 0"><b>${esc(v)}</b></td></tr>`).join("")}</table>
<p style="margin:10px 0 0;font-size:12px;color:#6B6861">This acknowledgment was given electronically in the Bar Lento schedule app after signing in with a personal PIN, and is recorded by Bar Lento with the date, time and device shown above.</p></div>
<p style="margin:16px 0 0;font-size:12px;color:#6B6861">Printable versions: <a href="https://bar-lento.vercel.app/docs.html">bar-lento.vercel.app/docs.html</a></p>
</div></div></body></html>`;
