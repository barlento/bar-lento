...
async function backfillFromToast(doc, fromISO) {
  ...
  [...weekKeys].sort().forEach((wk) => {
    ...
    // A week created only to hold clock-ins that are not there stays out of the app (empty "Past week" tabs are noise).
    if (created.has(wk) && !DAYS.some((d) => (data.weeks[wk][d] || []).length)) { delete data.weeks[wk]; created.delete(wk); return; }
