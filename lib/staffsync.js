...
async function syncFromToast(doc, opts) {
  ...
  try {
    ...
    data.staff.slice().forEach((n) => {
      ...
      delete data.birthdays[n]; delete map[n]; if (data.dept) delete data.dept[n];
  ...
  // Department from the Toast job titles, only where nothing is set yet (the manager's choice in the person sheet wins).
  data.dept = data.dept || {}; let deptChanged = false;
  { const byGuid = Object.fromEntries(emps.map((e) => [e.guid, e]));
    data.staff.forEach((n) => { if (data.dept[n]) return; const e = map[n] && byGuid[map[n]]; const d = e && toast.deptOf(e.jobs); if (d) { data.dept[n] = d; deptChanged = true; } });
