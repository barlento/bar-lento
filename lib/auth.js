...
async function setChefPassword(pw, name) {
  const store = require("./store");
  if (!pw) { await store._redis("DEL", CHEF_KEY); return false; }
  const salt = crypto.randomBytes(16).toString("hex");
  await store._redis("SET", CHEF_KEY, JSON.stringify({ salt, hash: scryptHex(pw, salt), name: name || null, at: new Date().toISOString() }));
  return true;
}
async function chefInfo() { const r = await chefRecord(); return r ? { on: true, name: r.name || null, at: r.at || null } : { on: false, name: null }; } // who holds the chef login (manager's Staff view)
...
module.exports = { adminEnabled, checkPassword, passwordFrom, roleOf, roleFrom, chefEnabled, chefInfo, setChefPassword, kitchenNames };
