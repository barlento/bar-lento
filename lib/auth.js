...
// The env manager password (Manager login button) is the service door only. No second password, no second button.
const MANAGER_DEPTS = new Set(["management", "owner"]);
function roleOfDept(dept) { return dept === "chef" ? "chef" : MANAGER_DEPTS.has(dept) ? "manager" : null; }
// "manager" | "chef" | null — from the password header, else from the staff token (the person's department)
async function roleFrom(req, doc) {
  if (!adminEnabled()) return null;
  if (checkPassword(passwordFrom(req))) return "manager";
  ...
  return name ? roleOfDept((d.data.dept || {})[name]) : null;
}
async function roleOf(candidate) { return adminEnabled() && checkPassword(candidate) ? "manager" : null; }
...
module.exports = { adminEnabled, checkPassword, passwordFrom, roleOf, roleFrom, roleOfDept, kitchenNames };
